...
interface RideRequest {
    _id: string;
    // Passenger can be an ID string OR an object { _id: string, name: string }
...
const ViewRequestScreen: React.FC = () => {
    ...
    const fetchRequests = useCallback(async (type: 'ride' | 'pickup', showAlerts = false) => {
        ...
        try {
            ...
            if (!token) {
                console.error('Authentication Error: Authentication token not found. Please log in again.');
                setIsLoading(false);
                return;
            }
            ...
            if (showAlerts && fetchedRequests.length === 0) {
                console.log(`No new nearby ${type} requests found at this time.`);
            }
        } catch (err: any) {
            ...
            if (isPickupStatusError) {
                 console.error('Status Error:', err.message || 'Your taxi must be in roaming status for pickup requests.');
            }
            else if (showAlerts || requests.length === 0) {
                 const isNotFoundError = err?.status === 404 || err?.message?.includes('not found');
                 if (!isNotFoundError){
                    console.error('Fetch Error:', err.message || `Failed to fetch ${type} requests. Please try again.`);
                 }
            }
            setRequests([]);
        } finally {
    ...
    const handleAccept = async (requestId: string) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('Success:', responseData.message || 'Request accepted! You can view details under "Accepted Passenger".');
                setRequests((prev) => prev.filter((req) => req._id !== requestId));
                navigation.navigate('AcceptedPassenger');
            } else {
                ...
                try {
                    ...
                    if (errorMsg.toLowerCase().includes("ride request not found")) {
                        console.error('Request Not Found: The requested ride was not found or might have been cancelled.');
                    } else if (errorMsg.toLowerCase().includes("request is no longer pending")) {
                        console.warn('Request Unavailable: This request is no longer pending and cannot be accepted.');
                        fetchRequests(viewType); // Refresh current list type
                    } else if (errorMsg.toLowerCase().includes("taxi for this driver not found")) {
                        console.error('Taxi Error: Your taxi information could not be found. Please contact support.');
                    } else if (errorMsg.toLowerCase().includes("taxi is not on the correct route")) {
                        console.error('Route Mismatch: Your taxi is not on the correct route for this request.');
                    } else if (errorMsg.toLowerCase().includes("taxi is not available for ride requests")) {
                        console.error('Taxi Unavailable: Your taxi is not currently available for ride requests.');
                    } else if (errorMsg.toLowerCase().includes("invalid route stops data")) {
                        console.error('Data Error: There was an issue with the route information. Please try again later.');
                    } else if (errorMsg.toLowerCase().includes("taxi has already passed the passenger's starting stop")) {
                        console.warn('Stop Passed: Your taxi has already passed the passenger\'s starting stop.');
                        fetchRequests(viewType); // Refresh current list type
                    } else if (errorMsg.toLowerCase().includes("taxi is not available for pickup requests")) {
                        console.error('Taxi Unavailable: Your taxi is not currently available for pickup requests.');
                    } else if (errorMsg.toLowerCase().includes("unsupported request type")) {
                        console.error('Unsupported Type: This request type is not supported.');
                    } else if (statusCode >= 500) {
                        console.error('Server Error: An error occurred on the server. Please try again later.');
                    } else {
                        console.error(`HTTP Error ${statusCode}:`, errorMsg);
                    }

                } catch (e) {
                    console.error("Failed to parse error response or map error message:", e);
                    console.error(`Request Failed (Status: ${statusCode}): An unexpected error occurred on the server.`);
                }
            }
        } catch (err: any) {
            console.error('Error accepting request:', err);
            if (err.message && err.message.includes('Network request failed')) {
                 console.error('Network Error: Could not connect to the server. Please check your internet connection.');
            } else {
                 console.error('Unexpected Error:', err.message || 'An unexpected error occurred while trying to accept the request.');
            }
        } finally {
            setIsAccepting(null);
        }
            ...
            <View style={styles.requestCardBody}>
                <View style={styles.requestInfoRow}>
                    ...
                    <Text style={styles.requestInfoValue}>
                        {item.passengerName
                            ? item.passengerName
                            : typeof item.passenger === 'object' && item.passenger !== null && item.passenger.name
                            ? item.passenger.name
