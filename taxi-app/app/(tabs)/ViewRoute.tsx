...
const RideRequestScreen: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!token) {
                 // Throwing here will direct to the catch block below for authentication errors
            ...
            // Make the API call
            // Assuming fetchData handles non-200 responses by either:
            // 1. Returning the parsed JSON body (including { error: "..." })
            // 2. Throwing an error object that might contain the parsed JSON body
            ...
            // This handles case 1: fetchData returned the error body directly
            if (response && typeof response === 'object' && response.error) {
                 console.error('Backend reported error (in response body):', response.error);
                 // Show the backend error message in the popup
                 showPopup('Submission Failed', 'error', response.error);
                 return; // Stop processing here
            }

            // If no backend error is found in the response body, assume success
            console.log('Request Response (Success):', response); // Log successful response data
            ...
            // Navigation and form clearing are handled by handleSuccessNavigation,
            // which is called by the popup's onClose prop when type is 'success'.

        } catch (err: any) {
            // This catch block handles network errors, issues within fetchData itself,
            // the authentication error thrown above, OR case 2: fetchData threw an error
            // that contains the backend response body.
            console.error('Error submitting request (caught):', err);

            let userMessage = 'Failed to submit request due to a system error.';
            let detailedMessage = err.message || 'An unknown error occurred.';

            // Attempt to extract backend error message if available in the caught error object
            // This structure depends heavily on how your fetchData function handles non-2xx responses
            // Common patterns: err.response.data.error, err.data.error, err.message containing the body string, etc.
            if (err.response && typeof err.response === 'object' && err.response.error) {
                 // Example: if fetchData throws an error like { response: { error: "..." } }
                 userMessage = 'Submission Failed (Backend Error)';
                 detailedMessage = err.response.error;
            } else if (err.data && typeof err.data === 'object' && err.data.error) {
                 // Example: if fetchData throws an error like { data: { error: "..." } } (e.g., Axios style)
                 userMessage = 'Submission Failed (Backend Error)';
                 detailedMessage = err.data.error;
            } else if (err.message && typeof err.message === 'string' && err.message.includes('Authentication failed')) {
                 // Specific check for the auth error thrown above
                 userMessage = 'Authentication Error';
                 detailedMessage = err.message;
            }
            // Add other checks here based on how your fetchData might structure errors
            // If none of the above match, userMessage and detailedMessage default to generic error

            showPopup('Submission Error', 'error', userMessage + '\nDetails: ' + detailedMessage);
